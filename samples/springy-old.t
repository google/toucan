...
class ParticleSystem {
  ParticleSystem(Body[]* b, Spring[]* s) {
    bodies = b;
    springs = s;
    x = new Pair[bodies.length];
    dx = new Pair[bodies.length];
    x1 = new Pair[bodies.length];
    x2 = new Pair[bodies.length];
    x3 = new Pair[bodies.length];
    k1 = new Pair[bodies.length];
    k2 = new Pair[bodies.length];
    k3 = new Pair[bodies.length];
    k4 = new Pair[bodies.length];
  }

  void getState(Pair[]^ p) {
  ...
  void derivEval(Pair[]^ result) {
    Vector gravity = Vector(0.0, -0.05);
    Vector wind = Vector(Math.rand() * 0.01, 0.0);
    for (int i = 0; i < bodies.length; ++i) {
    ...
    for (int i = 0; i < springs.length; ++i) {
      ...
      bodies[b1].applyForce(fk);
      bodies[b2].applyForce(-fk);
    }
    for (int i = 0; i < bodies.length; ++i) {
      bodies[i].derivEval();
...
RenderPipeline* springPipeline = new RenderPipeline<SpringShaders>(device, null, LineList);
